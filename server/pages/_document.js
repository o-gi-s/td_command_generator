...
CustomDocument.getInitialProps = async ctx => {
  const styledComponentSheets = new styled_components__WEBPACK_IMPORTED_MODULE_2__["ServerStyleSheet"]();
  const materialUiServerStyleSheets = new _material_ui_styles__WEBPACK_IMPORTED_MODULE_3__["ServerStyleSheets"]();
  const originalRenderPage = ctx.renderPage;

  try {
    ctx.renderPage = () => originalRenderPage({
      enhanceApp: App => props => styledComponentSheets.collectStyles(materialUiServerStyleSheets.collect(__jsx(App, props)))
    });

    const initialProps = await next_document__WEBPACK_IMPORTED_MODULE_1___default.a.getInitialProps(ctx);
    return _objectSpread(_objectSpread({}, initialProps), {}, {
      styles: __jsx(react__WEBPACK_IMPORTED_MODULE_0___default.a.Fragment, null, initialProps.styles, styledComponentSheets.getStyleElement(), materialUiServerStyleSheets.getStyleElement())
    });
  } finally {
